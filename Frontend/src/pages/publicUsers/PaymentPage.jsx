...
export default function PaymentPage() {
  ...
  const handleSubscribe = async (e) => {
    ...
    try {
      await api.post("/subscription/start", { plan: "THRUMPFIX_PRO" });
      // Always go to Login after payment — user logs back in and gets fresh PRO subscription
      navigate(route.Login, {
        state: { message: "🎉 Subscription activated! Please log in to continue." }
      });
    } catch (err) {
      ...
      // 403 "already have an active subscription" means they're on FREE_TRIAL
      // and the backend doesn't support upgrading via this endpoint.
      // ⚠️ Your backend needs a PATCH /subscription/upgrade endpoint for this case.
      // For now: show a clear message so they know what happened.
      if (status === 403 && message.toLowerCase().includes("already have an active subscription")) {
        setError("Your backend does not support upgrading from Free Trial to Pro yet. Please ask your backend developer to add a PATCH /subscription/upgrade endpoint.");
        setLoading(false);
        return;
      }
  ...
  return (
    <div className="min-h-screen bg-[#F1F5F9]">
      ...
      <div className="w-full max-w-4xl mx-auto px-6 sm:px-10 pt-7 pb-4">
        <button
          onClick={() => navigate(from === "dashboard" ? route.PlumberDashboard : from === "consumer-dashboard" ? route.ConsumerDashboard : route.Pricing)}
          className="flex items-center gap-1.5 text-[13px] font-semibold text-gray-700 hover:text-blue-600 transition-colors"
        >
          <svg width="14" height="14" viewBox="0 0 16 16" fill="none">
            <path d="M10 3L5 8l5 5" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
