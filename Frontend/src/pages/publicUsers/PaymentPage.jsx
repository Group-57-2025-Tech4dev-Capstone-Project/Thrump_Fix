...
export default function PaymentPage() {
  ...
  // SUBSCRIPTION FLOW:
    ...
    console.log("[PAYMENT] 🔵 Starting subscription process...");

    try {
      // First try to start subscription (works for brand new users)
      console.log("[PAYMENT] Trying POST /subscription/start with plan: THRUMPFIX_PRO");
      await api.post("/subscription/start", { plan: "THRUMPFIX_PRO" });

      console.log("[PAYMENT] ✅ Subscription started successfully");
      navigateToSuccess();
      ...
      if (status === 409) {
        ...
        try {
          ...
          console.log("[PAYMENT] ✅ Upgrade successful");
          navigateToSuccess();
        } catch (upgradeErr) {
