...
// ── Read user immediately from sessionStorage
...
export default function ConsumerDashboard() {
  ...
  const { user, jobs, loading, showOverlay, subscription } = state;
  ...
  const fetchSubscription = useCallback(async () => {
    try {
      ...
      dispatch({ type: "SET_SUBSCRIPTION", payload: res.data });
    } catch (err) {
  ...
  const fetchJobs = useCallback(async () => {
    try {
      ...
      const visible = list.filter(
        (job) => job.status?.toLowerCase() !== "logged"
      );
  ...
  const isNewUser = !user?.createdAt || (Date.now() - new Date(user.createdAt)) < 86400000;
